    ...
    <div class="bg-white p-10 rounded shadow-md w-full max-w-xs">
      <form @submit.prevent="submitForm">
        <div class="flex flex-col space-y-0">
          <div class="relative">
            <input type="text" v-model.trim.lazy="id" placeholder="아이디" @focus="handleFocus('id', true)" @blur="handleFocus('id', false)"
          ...
          <div class="relative">
            <input type="password" v-model.trim.lazy="password" placeholder="비밀번호" @focus="handleFocus('password', true)"
        ...
        <div class="relative w-full flex justify-center">
          <p v-if='showError' class='absolute top-2 text-red-500 font-mideum text-sm'>아이디와 비밀번호를 입력하세요.</p>
        </div>
        <button type="submit" class="mt-10 bg-blue-500 hover:bg-blue-700 w-full py-2 text-white font-semibold rounded">
          로그인
        </button>
      </form>
    </div>
