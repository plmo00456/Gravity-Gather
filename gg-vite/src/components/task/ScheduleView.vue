...
export default {
    ...
    return {
      ...
      calendarOptions: {
        ...
        eventDidMount: function (info) {
          ...
          const titleEl = info.el.querySelector(".fc-event-main > div");
          const titleElSpan = titleEl.querySelector(".title-span");
          if(titleElSpan && titleEl.scrollWidth > titleEl.clientWidth)
            titleEl.querySelector(".title-span").classList.add("truncate");
          ...
          }else if(titleElSpan.classList.contains("truncate")){
            titleEl.querySelector(".title-span").classList.add("truncate");
            this.utils.tippy(info.el, {
              content: this.highlightKeyword(info.event.title),
              allowHTML: true,
              theme: 'light',
            });
          }
          ...
          info.el.addEventListener("contextmenu", (e) => {
            e.preventDefault();
            if (info.event.extendedProps.user_seq !== this.user.seq) {
              return;
            }
        ...
        eventDrop: function (info) {
          ...
          start.setHours(this.utils.unixToFormat(oriStart, "%H"),
              this.utils.unixToFormat(oriStart, "%M"));
          end.setHours(this.utils.unixToFormat(oriEnd, "%H"),
              this.utils.unixToFormat(oriEnd, "%M"));
  ...
  setup() {
    ...
    const getCategories = async () => {
      try {
        ...
        console.error(error);
        instance.appContext.config.globalProperties.utils.msgError((error?.response?.data)
    ...
    const getTasks = async () => {
      if (instance.refs.fullCalendar) {
        instance.data.calendar.isLoading = true;
        ...
        try {
          ...
          if (taskStore?.dataResponse.status === 200) {
            ...
            instance.appContext.config.globalProperties.utils.msgError(this.dataResponse.data
                || instance.appContext.config.globalProperties.utils.normalErrorMsg);
          }
        ...
        instance.data.calendar.isLoading = false;
      }
    }
    ...
    onMounted(async () => {
      ...
      window.addEventListener('keyup', (e) => {
        if(instance.data.task.isShow != null) return;
        let flag = false;

        if (e.key === 'ArrowLeft') {
          flag = true;
          instance.data.calendarOptions.events = [];
      ...
      setTimeout(() => fullCalendarApi.updateSize(), 300);
    });

    const filteredTask = computed(() => {
      return tasks.value.filter(task => {
        return task.title.toLowerCase().includes(taskSearch.value)
            || task.content.toLowerCase().includes(taskSearch.value);
      });
    });
    watch(filteredTask, () => {
  ...
  methods: {
    handleDateClick: function (arg) {
      if (this.category.currentTab !== 'all' && this.category.currentTab !== 'important'
          && this.category.currentTab !== 'share') {
    ...
    toggleLeftSlide(){
      ...
      let calendarApi = this.$refs.fullCalendar.getApi();
      setTimeout(() => calendarApi.updateSize(), 500);
    },
    ...
    <div class="flex justify-center items-center w-full h-full bg-gray-300 bg-opacity-30">
      <transition name="slide-fade">
        <div v-if="category.isShow"
          ...
          <div class="flex">
            <div class="flex relative">
              <input type="text" placeholder="검색" v-model="taskSearch"
                     class="border text-sm pl-10 pr-4 py-2 rounded-l w-full focus:outline-none text-gray-400"
                     maxlength="20">
                ...
                class="flex items-center m-1 p-2 w-full rounded hover:bg-gray-200 cursor-pointer">
                        <span class="flex w-[13%]">
                            <font-awesome-icon
                                ...
                                class="self-start text-blue-300"
                                icon="fa-border-all"></font-awesome-icon>
                        </span>
                ...
                class="flex items-center m-1 p-2 w-full rounded hover:bg-gray-200 cursor-pointer">
                        <span class="flex w-[13%]">
                            <font-awesome-icon
                                ...
                                class="self-start text-yellow-400"
                ...
                class="flex items-center m-1 p-2 w-full rounded hover:bg-gray-200 cursor-pointer">
                        <span class="flex w-[13%]">
                            <font-awesome-icon
                                ...
                                class="self-start text-blue-400"
                                icon="fa-share-nodes"></font-awesome-icon>
                        </span>
            ...
            >
              <template #item="{ element }">
                <div
                  ...
                  <span class="flex w-[70%]"
                        v-if="category.editingItem !== element.seq">{{ element.category_nm }}</span>
                  ...
                  <div class="w-[15%] text-red-400 flex justify-center items-center group">
                    <font-awesome-icon
                        ...
                        class="font-bold p-1 opacity-0 group-hover:opacity-100"
                        icon="fa-trash-can"/>
                  </div>
                </div>
              </template>
          ...
          <div class="absolute bottom-0 left-0 w-full h-[2.5rem] flex items-center select-none"
               @click="addCategory">
            <div class="item flex items-center w-full px-3 rounded hover:bg-gray-200 cursor-pointer h-full">
              <span class="flex mr-2">
                  <font-awesome-icon class="self-start text-gray-400 hover:text-gray-600"
      ...
      <div class="h-[90%] flex flex-col w-full bg-white rounded p-6 border-yellow-500 border-t-[5px] relative">
        <div v-if='calendar.isLoading' class="absolute left-0 top-0 w-full h-full flex justify-center items-center bg-black bg-opacity-80 z-[9999]">
          <font-awesome-icon class="text-[5rem] text-blue-500 fa-spin" icon="fa-spinner"></font-awesome-icon>
        </div>
        <FullCalendar
            class="z-10"
            id="calendar"
      ...
      <form ref="taskForm" @submit.prevent="addTask">
        <div class="flex flex-col items-center p-5 w-full h-full">
          ...
          <p class="flex w-full h-[25rem] mb-3 rounded">
            <writeEditor ref="taskContent" class="w-full max-h-[25rem]"
          ...
          <div class="flex justify-between text-white w-full">
            <div>
              <button
                  class="px-5 py-3 bg-orange-500 rounded text-sm hover:bg-orange-400 mr-3 relative"
                  type="button"
                  @click="openShare">
